...
public static class Files {
    ...
    public static void CreateShortcut(
        ...
        if (!OperatingSystem.IsWindows())
            throw new PlatformNotSupportedException("Windows .lnk 快捷方式仅支持 Windows。");
    ...
    [Obsolete("Use PasteFromClipboardAsync(TopLevel, string, bool, bool) so Avalonia can access the platform clipboard.")]
    public static async Task<int> PasteFromClipboardAsync(string dest, bool copyFile, bool copyDir) {
        await Task.CompletedTask;
        return 0;
    }

    public static async Task<int> PasteFromClipboardAsync(TopLevel topLevel, string dest, bool copyFile, bool copyDir) {
        ArgumentNullException.ThrowIfNull(topLevel);
        if (string.IsNullOrEmpty(dest)) {
            throw new ArgumentException("Destination folder cannot be null or empty.", nameof(dest));
        }
        if (topLevel.Clipboard is null) return 0;

        if (!Directory.Exists(dest)) {
            Directory.CreateDirectory(dest);
        }

        var items = await topLevel.Clipboard.TryGetFilesAsync().ConfigureAwait(false);
        if (items is null || items.Length == 0) return 0;

        var count = 0;
        foreach (var item in items) {
            var sourcePath = item.TryGetLocalPath();
            if (string.IsNullOrEmpty(sourcePath)) continue;

            if (File.Exists(sourcePath)) {
                if (!copyFile) continue;
                var targetPath = Path.Combine(dest, Path.GetFileName(sourcePath));
                await CopyFileAsync(sourcePath, targetPath).ConfigureAwait(false);
                count++;
            } else if (Directory.Exists(sourcePath)) {
                if (!copyDir) continue;
                var targetPath = Path.Combine(dest, Path.GetFileName(sourcePath.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)));
                await CopyDirectoryAsync(sourcePath, targetPath).ConfigureAwait(false);
                count++;
            }
        }
