...
public static class Directories {
    /// <summary>
    /// 异步检查是否拥有对指定文件夹的读写权限。
    /// 如果文件夹不存在或没有权限，返回 false。会创建并删除一个临时文件以确认实际写入能力。
    /// </summary>
    ...
    public static async Task<bool> CheckPermissionAsync(string? path, CancellationToken cancellationToken = default) {
        try {
            ...
            await ProbeDirectoryAccessAsync(path, cancellationToken).ConfigureAwait(false);
            return true;
    ...
    private static bool IsSystemProtectedFolder(string path) {
        return OperatingSystem.IsWindows() &&
               (path.EndsWith(":\\System Volume Information", StringComparison.OrdinalIgnoreCase) ||
    ...
    private static async Task ProbeDirectoryAccessAsync(string path, CancellationToken cancellationToken) {
        await Task.Run(() => Directory.EnumerateFileSystemEntries(path, "*", SearchOption.TopDirectoryOnly).Any(), cancellationToken)
            .ConfigureAwait(false);

        var probe = Path.Combine(path, $".pcl-permission-test-{Guid.NewGuid():N}.tmp");
        try {
            ...
            try {
                ...
                LogWrapper.Warn(ex, $"删除权限检测临时文件失败：{probe}");
            }
        }
    }
