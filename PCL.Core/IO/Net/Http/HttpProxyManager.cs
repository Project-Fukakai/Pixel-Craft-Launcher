    ...
    public void RefreshSystemProxy()
    {
        lock (_lock)
        {
            try
            {
                if (!OperatingSystem.IsWindows())
                {
                    LogWrapper.Info("Proxy", "当前平台使用 .NET 默认系统代理。");
                    return;
                }
    ...
    public Uri? GetProxy(Uri destination)
    {
        lock (_lock)
        {
            return _mode switch
            {
                ...
                ProxyMode.SystemProxy => OperatingSystem.IsWindows()
                    ? _systemWebProxy.GetProxy(destination)
                    : WebRequest.DefaultWebProxy?.GetProxy(destination),
    ...
    public bool IsBypassed(Uri host)
    {
        lock (_lock)
        {
            return _mode switch
            {
                ...
                ProxyMode.SystemProxy => OperatingSystem.IsWindows()
                    ? _systemWebProxy.IsBypassed(host)
                    : WebRequest.DefaultWebProxy?.IsBypassed(host) ?? true,
