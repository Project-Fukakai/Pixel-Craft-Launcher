...
public class CatColorResource
{
    ...
    public void Apply()
    {
        ...
        var color = Color.FromArgb(ToByte(Alpha), ToByte(Red), ToByte(Green), ToByte(Blue));
        if (ApplyToColor) res[$"{ColorPrefix}{Suffix}"] = color;
        if (ApplyToBrush) res[$"{BrushPrefix}{Suffix}"] = new SolidColorBrush(color);
    }

    private static byte ToByte(float value) => (byte)System.Math.Clamp((int)System.Math.Round(value * 255), 0, 255);
}
