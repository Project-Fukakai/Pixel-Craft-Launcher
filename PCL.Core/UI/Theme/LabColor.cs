...
public class LabColor
{
    ...
    /// <summary>
    /// Create a <see cref="LabColor"/> instance from <see cref="Color"/>.
    /// </summary>
    public static LabColor FromWpfColor(Color color)
        => FromRgb(color.R / 255d, color.G / 255d, color.B / 255d, color.A / 255d);

    public static implicit operator LabColor(Color color) => FromWpfColor(color);
    ...
    public static implicit operator Color(LabColor lab) => lab.ToWpfColor();

    private static byte ToByte(float value) => (byte)Math.Clamp((int)Math.Round(value * 255), 0, 255);
