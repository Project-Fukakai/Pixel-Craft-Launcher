...
{
    public static async Task<bool> StartLoginAsync(CancellationToken cancellationToken = default)
    {
        ...
        var completed = await Task.WhenAny(completion.Task, Task.Delay(TimeSpan.FromMinutes(3), cancellationToken)).ConfigureAwait(false);
