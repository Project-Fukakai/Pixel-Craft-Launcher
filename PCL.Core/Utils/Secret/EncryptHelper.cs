...
#pragma warning restore CS0612,CS0618 // Type or member is obsolete
    ...
    private static byte[] _GetKey()
    {
        ...
        if (File.Exists(keyFile))
        {
            ...
            {
                1 when OperatingSystem.IsWindows() => ProtectedData.Unprotect(data.Data, _IdentifyEntropy, DataProtectionScope.CurrentUser),
                2 => data.Data,
        ...
        else
        {
            ...
            {
                Version = OperatingSystem.IsWindows() ? 1u : 2u,
                Data = OperatingSystem.IsWindows()
                    ? ProtectedData.Protect(randomKey, _IdentifyEntropy, DataProtectionScope.CurrentUser)
                    : randomKey
            });
            ...
            TryRestrictFilePermissions(tmpFile);

            File.Move(tmpFile, keyFile, true);
            TryRestrictFilePermissions(keyFile);
    ...
    private static void TryRestrictFilePermissions(string path)
    {
        ...
        {
            // Best-effort only; encryption key creation must still work on file systems without Unix modes.
        }
    }
