...
public class Identify
{
    ...
    private static byte[] _GetRawId()
    {
        ...
        try
        {
            ...
            {
                code.Append("Machine:").Append(ReadFirstExistingFile("/etc/machine-id", "/var/lib/dbus/machine-id"))
                    .Append("|Host:").Append(Environment.MachineName)
                    .Append("|User:").Append(Environment.UserName)
                    .Append("|Home:").Append(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile))
                    .Append("|OS:").Append(RuntimeInformation.OSDescription)
                    .Append("|Arch:").Append(RuntimeInformation.OSArchitecture);
            }
        }
        catch (Exception ex)
        {
    ...
    private static string _getLauncherId()
    {
        try
        {
            var prefix = "Pixel Craft Launcher|"u8.ToArray();
            var ctx = RawId;
