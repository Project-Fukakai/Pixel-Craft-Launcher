...
public static class SystemInfo
{
    ...
    public static string CPUName = RuntimeInformation.ProcessArchitecture.ToString();
        ...
        lock (_lock)
        {
            ...
            try
            {
                if (OperatingSystem.IsWindows())
                {
                    ...
                    {
                        CPUName = queryObj["Name"]?.ToString()?.Trim() ?? CPUName;
                        break; // 通常只需要第一个CPU的信息
                    }
                }
                else
                {
                    CPUName = $"{RuntimeInformation.ProcessArchitecture} ({Environment.ProcessorCount} cores)";
                }
            }
            catch (Exception ex)
            {
