...
public class DefaultPathsScanner : IJavaScanner
{
    ...
    private static HashSet<string> _GetSearchRoots()
    {
        ...
        {
            foreach (var path in _GetUnixLikeSearchRoots())
            {
    ...
    private static void _BfsSearch(string rootPath, ICollection<string> results)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            try
            {
                ...
                {
                    var javaExe = JavaScannerUtils.GetJavaExecutables(dir).FirstOrDefault();
                    if (javaExe is not null)
                    {
                        results.Add(javaExe);
                    }
                    else
    ...
    private static IEnumerable<string> _GetUnixLikeSearchRoots()
    {
